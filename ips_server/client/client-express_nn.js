...
function main() {
  ...
  var client = new putData2DB_proto.Putter('grpc-server:50051', grpc.credentials.createInsecure());
  ...
  app.post('/appdata', function(req, res, next) {
    ...
    let devserial = "";
    let rssi_avg = ["106","106","106","106","106","106","106","106","106"];
    let scAddr;
    let scTime = [];
    let sum = 0;
    let avgRssi = 0;
        ...
        if(scDev[i].address == addr2[u]){
          ...
          for(var k = 0; k < scDev[i].rssi.length; k++){
            ...
            if(k == scDev[i].rssi.length - 1){
              avgRssi = sum/scDev[i].rssi.length * -1;
    ...
    client.putData2DB({dev: dev , x: smX, y: smY, devserial: devserial, addr: addr2, rssi: rssi_avg}, function(err, response) {
  ...
  app.post('/putdata', function (req, res) {
    ...
    let direction = "";
    let devserial = "";
    let rssi = ["106","106","106","106","106","106","106","106","106"];
    let addr = [
            "d1:c5:f3:61:26:1d",
            "d2:e0:a9:d9:04:22",
            "d8:dc:4b:36:5f:ee",
    ...
    for (var i = 0; i < addr.length; i++) {
      ...
      for (var j = 0; j < scannedAddr.length; j++) {
        if(scannedAddr[j] == addr[i]){
          rssi[i] =scannedRssi[j]*-1;
        }
      }
      // if(i == addr.length-1) {       
      //   console.log(scannedAddr)
      //   console.log("time to send data")
      // }
    }

    client.putData2DB({dev: dev , x: x, y: y, devserial: devserial, addr: addr, rssi: rssi}, function(err, response) {
      console.log('Response from grpc-server:', response.message);
      //res.send(response.message);
    });
